...
public class FFmpeg extends ExecutableCommand {
	...
	private static void initialize() throws IOException, UnknownOperatingSystem {
		...
		if(os.contains("mac"))
			path = path + "/mac/ffmpeg";
		else if(os.contains("win")) {
			if(os.contains("64"))
				path = path + "\\windows64\\ffmpeg.exe";
			else
				path = path + "\\windows32\\ffmpeg.exe";	
		}
	 ...
	 * @param verbose set it to true to display the console output of ffmpeg into the system output
	 ...
	 */
	public static int execute(boolean verbose, String[] arguments) throws IOException, InterruptedException, UnknownOperatingSystem {
	...
	public static int concatenateVideos(FileExtended outputVideoFile, FileExtended... inputVideoFiles) throws IOException, InterruptedException, UnknownOperatingSystem {
		...
		for(int i = 0; i < inputVideoFiles.length; i++) {
			...
			if(!inputVideo.getName().endsWith("ts")) {
				...
				if(execute(false, arguments.split(" ")) != 0) 
					return -1;
		...
		int exitcode = execute(false, arguments.split(" "));
